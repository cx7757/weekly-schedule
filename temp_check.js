...
function toggleDone(dayKey,idx){
  ...
  if(!wasDone && t.prio<3){
    t.prio=t.prio+1;
    const starMap={1:"⭐",2:"⭐⭐",3:"⭐⭐⭐"};
    showToast(`✅ 已完成！升级到 ${starMap[t.prio]}`);
  } else {
...
function saveEdit(){
  ...
  ts[editIdx]={...ts[editIdx],name,time,tag,prio,eisen};
  save();render();closeModal();showToast("✏️ 已保存");
}
function deleteTask(){
  if(!confirm("确定删除这个任务吗？"))return;
  getTasksOf(weekOffset,editKey).splice(editIdx,1);
  save();render();closeModal();showToast("🗑️ 已删除");
}
...
function closeNotes(){
  ...
  if(sv)sv.style.display='';if(pd)pd.style.display='';if(dt)dt.style.display='';if(ma)ma.style.display='';if(tb)tb.style.display='';
}
function notesThemeSync(){
  const isDark=document.documentElement.getAttribute('data-theme')==='dark';
  const btn=document.getElementById('notesThemeBtn');if(btn)btn.textContent=isDark?'☀️':'🌙';
}

function goHome(){
  notesCat=null;notesSub=null;notesSearchMode=false;
  document.getElementById('notesSearchInput').value='';
  document.getElementById('notesPageTitle').textContent='📚 考公知识速记';
  document.getElementById('notesFab').style.display='none';
  document.getElementById('notesFilter').textContent='';
  document.getElementById('notesNav').innerHTML='';
  let html='<div class="notes-home-grid">';
  for(const cat of Object.keys(NOTES_DB)){
    const total=notesGetCatTotal(cat),color=NOTES_COLORS[cat]||'#4a6cf7',icon=NOTES_ICONS[cat]||'📌';
    html+=`<div class="notes-home-card" onclick="openNotesCat('${cat}')"><div class="notes-home-card-icon">${icon}</div><div class="notes-home-card-name">${cat}</div><div class="notes-home-card-count">${total} 条</div></div>`;
  }
  html+='</div>';
  document.getElementById('notesContent').innerHTML=html;
}

function openNotesCat(cat){
  notesCat=cat;notesSub=null;notesSearchMode=false;
  document.getElementById('notesSearchInput').value='';
  document.getElementById('notesFilter').textContent='';
  document.getElementById('notesPageTitle').textContent=(NOTES_ICONS[cat]||'')+' '+cat;
  document.getElementById('notesFab').style.display='flex';
  const subs=Object.keys(NOTES_DB[cat]);
  if(subs.length===0){
    document.getElementById('notesNav').innerHTML=`<div class="notes-breadcrumb"><span onclick="goHome()" style="cursor:pointer">首页</span> › <span class="bc-active">${cat}</span></div>`;
    document.getElementById('notesContent').innerHTML='<div class="notes-empty"><div class="notes-empty-icon">📭</div>暂无数据，待补充</div>';return;
  }
  renderNotesCatView(cat);
}

function renderNotesCatView(cat){
  const subs=Object.keys(NOTES_DB[cat]),total=notesGetCatTotal(cat);
  document.getElementById('notesNav').innerHTML=`<div class="notes-breadcrumb"><span onclick="goHome()" style="cursor:pointer">首页</span> › <span class="bc-active">${cat}</span></div><div class="notes-sub-tabs">${subs.map(s=>{const cnt=notesGetSubTotal(cat,s);const isActive=(!notesSub&&s===subs[0])||notesSub===s;return`<div class="notes-sub-tab ${isActive?'active':''}" onclick="selectNotesSub('${cat}','${s}')">${s}(${cnt})</div>`;}).join('')}</div>`;
  if(!notesSub||!NOTES_DB[cat][notesSub])notesSub=subs[0];
  renderNotesSubContent(cat,notesSub);
}

function selectNotesSub(cat,sub){
  notesSub=sub;
  const tabs=document.querySelectorAll('.notes-sub-tab');tabs.forEach(t=>t.classList.remove('active'));
  if(event&&event.target)event.target.classList.add('active');
  renderNotesSubContent(cat,sub);
}

function renderNotesSubContent(cat,sub){
  const items=NOTES_DB[cat][sub]||[];
  if(items.length===0){document.getElementById('notesContent').innerHTML='<div class="notes-empty"><div class="notes-empty-icon">📭</div>该分类暂无数据，待补充</div>';return;}
  let html=`<div class="notes-section"><div class="notes-section-title" style="border-color:${NOTES_COLORS[cat]}">📖 ${sub} <span class="notes-section-count">${items.length}条</span></div>`;
  for(let i=0;i<items.length;i++)html+=`<div class="notes-item" data-cat="${cat}"><span class="notes-item-num">${i+1}.</span>${items[i].t}</div>`;
  html+='</div>';
  document.getElementById('notesContent').innerHTML=html;
}
...
function onNotesSearch(){
  ...
  if(!q){if(notesCat)openNotesCat(notesCat);else goHome();return;}
  notesSearchMode=true;
  document.getElementById('notesFilter').textContent='';
  document.getElementById('notesNav').innerHTML='';
  document.getElementById('notesPageTitle').textContent='🔍 搜索结果';
  let totalMatches=0,html='';
  for(const cat of Object.keys(NOTES_DB)){
    for(const sub of Object.keys(NOTES_DB[cat])){
      const items=NOTES_DB[cat][sub].filter(it=>it.t.toLowerCase().includes(q));
      if(items.length===0)continue;totalMatches+=items.length;
      html+=`<div class="notes-section"><div class="notes-section-title" style="border-color:${NOTES_COLORS[cat]}">${NOTES_ICONS[cat]} ${cat} › ${sub} <span class="notes-section-count">匹配${items.length}条</span></div>`;
      for(let i=0;i<items.length;i++){let text=items[i].t;const re=new RegExp(`(${q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')})`,'gi');text=text.replace(re,'<mark>$1</mark>');html+=`<div class="notes-item" data-cat="${cat}"><span class="notes-item-num">${i+1}.</span>${text}</div>`;}
      html+='</div>';
    }
  }
  if(!html)html='<div class="notes-empty"><div class="notes-empty-icon">🔍</div>没有匹配的知识点</div>';
  document.getElementById('notesFilter').textContent=`找到 ${totalMatches} 条`;
  document.getElementById('notesContent').innerHTML=html;
}

// 🏠按钮：从资料库首页回到周计划
function notesBackToSchedule(){closeNotes();}

// ── Init ──
load();initTheme();calcInitWeek();currentDay=todayDayKey();render();initPWA();
