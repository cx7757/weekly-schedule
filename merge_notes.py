# -*- coding: utf-8 -*-
"""
将考公资料库精确合并到V7 index.html中。
从v7_good_backup.html读取，输出到index.html。
4个插入点，不触碰V7已有代码。
"""
import json

SRC = 'c:/Users/chenx/WorkBuddy/Claw/v7_good_backup.html'
DST = 'c:/Users/chenx/WorkBuddy/Claw/index.html'
...
# ===== 3. HTML: 在 <!-- Edit Modal --> 之前插入 =====
NOTES_HTML = """
<!-- Notes View (hidden by default) -->
<div id="notesView" style="display:none">
  <div class="notes-body" id="notesContent"></div>
  <button class="notes-home-btn" onclick="goHome()" title="返回首页" style="display:none" id="notesHomeBtn">🏠</button>
</div>

"""

pos = html.find('<!-- Edit Modal -->')
html = html[:pos] + NOTES_HTML + '\n' + html[pos:]
...
function renderNotesHome(){
  ...
  grid.innerHTML=cats.map(function(c){
    var cnt=notesGetCatTotal(c);
    var icon=NOTES_ICONS[c]||'📖';
    var color=NOTES_COLORS[c]||'#6c63ff';
...
function openNotesCat(c){
  ...
  var el=document.getElementById('notesContent');
  var total=notesGetCatTotal(c);
  var subs=Object.keys(NOTES_DB[c]);
  var icon=NOTES_ICONS[c]||'📖';
  var color=NOTES_COLORS[c]||'#6c63ff';
...
function onNotesSearch(){
    ...
    subs.forEach(function(s){
      ...
      NOTES_DB[c][s].forEach(function(item,idx){
        if(item.t.toLowerCase().indexOf(q)>=0){
          results.push({c:c,s:s,i:idx,t:item.t});
        }
      });
    });
  });
  ...
  if(!list){return;}
  if(results.length===0){
    list.innerHTML='<div class="notes-item" style="color:var(--sub);text-align:center">未找到匹配的知识点</div>';
    return;
  }
  list.innerHTML=results.map(function(r){
    var color=NOTES_COLORS[r.c]||'#6c63ff';
    var highlighted=r.t.replace(new RegExp('('+q.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')+')','gi'),'<mark>$1</mark>');
...
# Verify
checks = [
    ('function render', 'V7 render'),
    ('function toggleTheme', 'V7 toggleTheme'),
    ('function submitAdd', 'V7 submitAdd'),
    ('function openNotes', 'Notes openNotes'),
    ('function closeNotes', 'Notes closeNotes'),
    ('const NOTES_DB', 'Notes DB'),
    ('// ── Init ──', 'Init section'),
    ('</style>', 'CSS close'),
    ('<!-- Edit Modal -->', 'Edit Modal'),
    ('id="notesView"', 'Notes HTML'),
    ('onclick="openNotes()"', 'Notes button'),
]
for text, name in checks:
    idx = html.find(text)
    status = 'OK' if idx >= 0 else 'MISSING!'
    print(f'  {name}: {status} (pos {idx})')
